...
(function (root, factory) {
    ...
    function initSandBox(
	    ...
	    function SandboxInfo(params) {
		    validateTypesInPrivateCode(params, _.isPlainObject, 'params should be an object');
		    validateTypesInPrivateCode(params.prefix, _.isString, 'params.prefix should be a string');
		    validateTypesInPrivateCode(params.childrenPrefix, _.isString, 'params.childrenPrefix should be a string');
		    validateTypesInPrivateCode(
			    params.parentInfo,
			    function (value) {
				    return _.isNull(value) || isSandboxInfo(value);
		        },
			    'params.parentInfo should be a string'
		    );
	    ...
	    PrivateData.prototype.addListener = function addListener(listenerInfo) {
		    validateTypesInPrivateCode(
			    listenerInfo,
			    isSandboxListener,
			    'listenerInfo should be an instance of SandboxListener'
		    );
	    ...
	    PrivateData.getByRegExp = function getPrivateDataByRegExp(regExpOrArray) {
		    ...
		    validateTypesInPrivateCode(
			    regExes,
			    function (value) {
				    return _.all(value, _.isRegExp);
			    },
			    'regExes should be an array of regular expressions'
		    );
	    ...
	    function SandboxListener(params) {
		    validateTypesInPrivateCode(params, _.isPlainObject, 'params should be an object');
		    validateTypesInPrivateCode(params.event, _.isString, 'params.event should be a string');
		    validateTypesInPrivateCode(params.handler, _.isFunction, 'params.handler should be a function');
		    validateTypesInPrivateCode(params.binding, _.isObject, 'params.binding should be a function');
	    ...
	    SandboxListener.process = function processSandboxListener(listener, candidateData, candidatePrefix) {
		    validateTypesInPrivateCode(listener, isSandboxListener,
			    'listener should be an instance of SandboxListener');
		    validateTypesInPrivateCode(listener, isSandboxListener,
			    'listener should be an instance of SandboxListener');
		    validateTypesInPrivateCode(candidatePrefix, _.isString, 'candidatePrefix should be a string');
	    ...
	    SandboxListener.callEventListeners = function callEventListeners(listeners, event) {
		    validateTypesInPrivateCode(
			    listeners,
			    function (value) {
			        return _.all(value, isSandboxListener);
		        },
			    'listeners should be an array of SandboxListener instances'
		    );
		    validateTypesInPrivateCode(event, isCachedEvent, 'event should be an instance of CachedEvent');
	    ...
	    CandidatesData.prototype.addBy = function addCandidatesDataBy(prefixes) {
		    validateTypesInPrivateCode(
			    prefixes,
			    function (prefixes) {
				    return _.all(prefixes, function (prefix) {
					    return _.isString(prefix) || _.isRegExp(prefix) || _.all(prefix, _.isRegExp);
				    });
			    },
			    'prefixes should be {Array.<String|RegExp|RegExp[]>}'
		    );
		    var that = this;
	    ...
	    CandidatesData.prototype.getFilteredByPrefix = function getFilteredCandidatesByPrefix(prefixes) {
		    validateTypesInPrivateCode(
			    prefixes,
			    function (prefixes) {
				    return _.all(prefixes, _.isString);
			    },
			    'prefixes should be a string'
		    );
		    return _.pick(
			    this.storage,
	    ...
	    CandidatesData.getCandidatesForPermissions = function getCandidatesForPermissions(sandbox, target) {
		    validateTypesInPrivateCode(sandbox, isSandbox, 'sandbox should be an instance of Sandbox');
	    ...
	    CandidatesData.updatePermissions = function updateCandidatesPermissions(updater, candidates, permissionsMap) {
		    validateTypesInPrivateCode(updater, _.isFunction, 'updater should be a function');
		    validateTypesInPrivateCode(
			    candidates,
			    function (candidates) {
				    return _.all(candidates, function (candidate) {
					    return isPrivateData(candidate);
				    });
			    },
			    '{Object.<String, PrivateData[]>} candidates'
		    );
		    validateTypesInPrivateCode(
			    permissionsMap,
			    function (permissionsMap) {
				    return _.all(permissionsMap, _.isString);
			    },
	    ...
	    /**
	     * checks that value is a SandboxListener instance
	     * @param {*} value
	     * @return {boolean}
	     */
	    function isSandboxListener(value) {
		    return value instanceof SandboxListener;
	    }
	    /**
	     * checks that value is a SandboxInfo instance
	     * @param {*} value
	     * @return {boolean}
	     */
	    function isSandboxInfo(value) {
		    return value instanceof SandboxInfo;
	    }
	    /**
	     * checks that value is a PrivateData instance
	     * @param {*} value
	     * @return {boolean}
	     */
	    function isPrivateData(value) {
		    return value instanceof PrivateData;
	    }
