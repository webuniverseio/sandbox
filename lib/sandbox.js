        ...
        var Sandbox = (function () {
			        ...
			        if (result && cacheSettings.expire) {
				        if (cachedEvent.invalidate) {
					        ...
					        timingFn = _[timingFnName](function () {
						        removeFromCache(parentData, cachedEvent);
            ...
            var emit = function (event, data) {
		        var that = this,
			        thisData = getInstanceData(that),
			        candidates = {},
			        privateData = getPrivateData(thisData.prefix),
			        parentPrefix = thisData.prefix.replace(
				        new RegExp(thisData.parentData.childPrefix + thisData.name),
				        ''
			        ),
			        childrenPrefixRegex = new RegExp(
				        thisData.prefix.replace(new RegExp(thisData.name + '$'), '') + '[^/]*$'
			        ),
			        parentPrivateData = getPrivateData(parentPrefix);
		        candidates[thisData.prefix] = privateData;
		        candidates[parentPrefix] = parentPrivateData;
		        _.each(getPrivateDataByRegex(childrenPrefixRegex), function (itemData, itemPrefix) {
			        candidates[itemPrefix] = itemData;
		        });

		        //TODO: then based on permissions add data to cache

		        privateData.cache.push({
		            event: event,
		            data: data,
		            invalidate: null //TODO: change to false
	            });
	        ...
	        var off = function (event) {
		        var thisData = getInstanceData(this),
			        name = thisData.name;
		        _.remove(thisData.listeners, function (listener) {
