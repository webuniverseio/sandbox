    ...
    function initSandBox(
	    ...
	    Sandbox.prototype.settings = function setSandboxSettings(settings) {
		    validateTypes({
			    ...
			    errorMessage: 'settings should be an object'
		    });
	    ...
	    Sandbox.prototype.on = function sandboxOn(eventName, handler, thisBinding) {
		    validateTypes([{
			    ...
			    message: 'eventName should be a string'
		    }, {
			    value: handler,
	    ...
	    var updatePermissions = function updatePermissions(target, permissionsMap, updater) {
		    ...
		    validateTypes([{
			    ...
			    validator: _.isObject,
			    message: 'permissionsMap should be an object'
		    }, {
			    value: updater,
			    validator: _.isFunction,
			    message: 'updater should be a function'
		    }]);
	    ...
	    function setCurrentParent(parent) {
		    validateTypes({
			    value: parent,
			    validator: isSandbox,
			    message: 'parent should be an instance of Sandbox'
		    });
	    ...
	    function SandboxInfo(params) {
		    validateTypes({
			    value: params,
			    validator: function validateParams(value) {
				    return _.isObject(value) && _.isString(value.name) &&
				           _.isString(value.prefix) && _.isString(value.childrenPrefix);
			    },
			    message: 'check signature {name: String, prefix: String, childrenPrefix: string}'
		    });
	    ...
	    /**
	     * @param {Object[]|Object} varsAndValidators
	     * @throws SandboxError
	     */
	    var validateTypes = function validateTypes(varsAndValidators) {
		    varsAndValidators = ensureIsArray(varsAndValidators);
		    var errors = _.filter(varsAndValidators, function (/**TypeValidatorInfo*/pair) {
			    return !pair.validator(pair.value);
		    });
		    if (errors.length) {
			    throw new SandboxError(_.map(errors, function (/**TypeValidatorInfo*/pair) {
				    return pair.errorMessage + ', received type: ' + typeof pair.value;
			    }).join('; '));
		    }
	    };
	    //that looks a bit weird, but basically allows to alter build
	    if (window && window.SandboxFlags && SandboxFlags['noValidation']) {
		    validateTypes = _.noop;
	    }
