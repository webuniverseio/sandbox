...
(function (root, factory) {
    ...
    function initSandBox(
	    ...
	    var updatePermissions = function updatePermissions(target, permissionsMap, updater) {
		    ...
		    var candidates = CandidatesData.getCandidatesForPermissions(this, target);
	     ...
	     * @param {Object.<String, String[]>} permissionsMap permissions map
	     */
	    Sandbox.prototype.grant = _.partialRight(updatePermissions, simplePermissions.grant);
	    /**
	     * revokes permissions
	     * @param {String|String[]} [from] sandbox name(s) to revoke permissions from
	     * @param {Object.<String, String[]>} permissionsMap permissions map
	     */
	    Sandbox.prototype.revoke = _.partialRight(updatePermissions, simplePermissions.revoke);
	    /**
	    ...
	    CandidatesData.getCandidatesForPermissions = function getCandidatesForPermissions(sandbox, target) {
		    ...
		    //TODO: refactor how you retrieve cache and possibly listeners using permissions
		    //TODO: remap permissionsMap keys
	    ...
	    CandidatesData.updatePermissions = function updateCandidatesPermissions(updater, candidates, permissionsMap) {
		    _.each(candidates, function (data, prefix) {
			    updater(data.permissions, prefix, permissionsMap);
		    });
	    };
	    ...
	    CachedEvent.prototype.setupExpirationFor = function setupEventExpirationFor(candidateData) {
		    ...
		    //TODO: this part is a bit hard to read
	    ...
	    CachedEvent.prototype.whenUnhandledStoreIn = function whenUnhandledStoreEventIn(candidateData) {
		    ...
		    if (candidateListeners.length) {
			    storeInCache = false;
			    var eventListeners = _.filter(candidateListeners, {event: cachedEvent.event});
			    SandboxListener.callEventListeners(eventListeners, cachedEvent);
		    }
		    if (storeInCache) {
	     ...
	     * @return {string}
	     */
	    CachedEvent.getTimingFnName = function getTimingFnName(cacheSettings) {
         ...
         * @return {boolean}
         */
        function hasPermitByPrefixes(source, permit, sourcePrefix) {
	        var permissionsMapForPrefix = _.find(source, function findByPrefixes(/**Entry*/entry) {
		        return sourcePrefix === entry.target;
	        });
