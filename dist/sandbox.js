...
(function (root, factory) {
    ...
    if (typeof define === 'function' && define.amd) {
        ...
        var lodashModuleName = isNode ? 'lodash' : '_';
    ...
    function initSandBox(/*Object*/exports,
        ...
        /**
         * @name SandboxExports
         * @type {{constructor: Sandbox, errorConstructor: SandboxError}}
         */
        ...
        function Sandbox(name, data) {
            ...
            var promise = new PromisedSandbox();
            var existingSandbox = promise.checkForPrefix(prefix);
            if (existingSandbox) {
                promise.setData(data);
                return existingSandbox;
            }

            ensureUniqueSiblingsNames(prefix, name);
        ...
        /**
         * @class PromisedSandbox
         */
        function PromisedSandbox() {
            this.sandbox = null;
        }
         ...
         * @return {(null|Sandbox)}
         */
        PromisedSandbox.prototype.checkForPrefix = function checkIfSandboxIsPromised(prefix) {
            var sandbox = _.find(instanceReference, function (sandbox) {
                return sandbox.data() === prefix;
            });
            if (sandbox) {
         ...
         * @param {*} data
         */
        PromisedSandbox.prototype.setData = function setDataForPromisedSandbox(data) {
            if (this.sandbox) {
                var info = SandboxInfo.getForSandbox(this.sandbox);
                info.data = data;
            }
        };
        ...
        var updatePermissions = function updatePermissions(target, permissionsMap, updater) {
            ...
            var that = this;
            ensureSandboxCreated(_.keys(permissionsMap).concat(target), that);
            var info = SandboxInfo.getForSandbox(that);
            var candidates = new CandidatesData();
         ...
         */
        function ensureSandboxCreated(names, sandbox) {
            names = ensureIsArray(names);
            var info = SandboxInfo.getForSandbox(sandbox);
            var prefixes = getPrefixForName(names, info);
            _.each(prefixes, function (prefix, position) {
                if (!(prefix in privateDataStorage)) {
                    sandbox.kid(names[position], prefix);
                }
            });
        }
         ...
         * @throws SandboxError
         */
        function ensureUniqueSiblingsNames(prefix, name) {
            if (prefix in privateDataStorage) {
        ...
        PrivateData.prototype.unMapFromIndex = function unMapPrivateDataFrom(prefix) {
            privateDataStorage = _.omit(privateDataStorage, prefix);
        };
        /**
        ...
        PrivateData.getByRegExp = function getPrivateDataByRegExp(regExpOrArray) {
            ...
            return _.pickBy(privateDataStorage, function (data, prefix) {
...
* settings: CacheSettings}} params
        ...
        function isCollectionValid(validator) {
            return function (collection) {
                return _.every(collection, validator);
            };
        }
        /**
        ...
        function validateCollectionValuesAsStrings(collection) {
            return _.isObject(collection) && _.every(collection, _.isString);
        }
        ...
        exports.constructor = Sandbox;
        exports.errorConstructor = SandboxError;
    })
);